...
teacherRouter.get("/classes/:classId/students",
...
teacherRouter.get("/students/:studentId",
  validateTeacherToken,
  async (req, res) => {
    try {
...
teacherRouter.post("/classes/:classId/add-students",
    validateTeacherToken,
    async (req, res) => {
      const teacherId = req.teacher._id;
      const classId = req.params.classId;
      ...
      try {
        const classData = await ClassModel.findOne({ _id: classId, teacher: teacherId });
        if (!classData) {
          return res.status(404).json({
            message: "Không tìm thấy lớp hoặc bạn không có quyền truy cập lớp này",
          });
        }
...
teacherRouter.delete("/classes/:classId/remove-student/:studentId",
    validateTeacherToken,
    async (req, res) => {
